...
class EventHandler {
    ...
    async getEvents(query) {
        let queryBuilder = this.#client
            ...
            ...(query["with-ticket-count"] === true
                ? {
                    ticketCount: this.#client.$count(ticketTable, eq(this.#table.id, ticketTable.eventId)),
                }
                : {}),
        })
            .from(this.#table)
        ...
        if (query["user-id"] !== undefined) {
            queryBuilder = queryBuilder.where(eq(this.#table.creatorId, query["user-id"]));
        }
    ...
    async updateEvent(userId, eventId, updatedEvent) {
        const eventsReturned = await this.#client
            .update(this.#table)
            .set(updatedEvent)
            .where(and(eq(this.#table.creatorId, userId), eq(this.#table.id, eventId)))
            .returning();
        return eventsReturned.at(0);
    }
    /**
...
export { EventHandler, eventInsertSchema, eventUpdateSchema, eventSelectSchema, };
